...
//        return jdbcTemplate.queryForInt("SELECT COUNT(*) FROM dba_tablespaces WHERE tablespace_name=?", name) > 0;
        return jdbcTemplate.queryForInt("select COUNT(*) from all_objects where object_type = 'SCH' AND object_name =?", name) > 0;
    }

    @Override
    protected boolean doEmpty() throws SQLException {
//        return !DmSchema.ObjectType.supportedTypesExist(jdbcTemplate, database, this);
        return jdbcTemplate.queryForInt("select COUNT(*) from dba_tables where owner=?", name) == 0;
    }
